...
func (p *BlobPool) recheck(addr common.Address, inclusions map[common.Hash]uint64) {
	...
	if txs[0].nonce < next {
		...
		for len(txs) > 0 && txs[0].nonce < next {
			...
			p.trackRemoved(txs[0].hash, "nonce expired")
		...
		for addr, txs := range p.index {
			for i, tx := range txs {
				if tx.execTipCap.Cmp(newTip) < 0 {
					...
					p.lookup.untrack(tx)
					p.trackRemoved(tx.hash, "underpriced")
					txs[i] = nil
...
func (p *BlobPool) addLocked(tx *types.Transaction, checkGapped bool) (err error) {
	...
	if len(p.index[from]) > offset {
		...
		p.trackRemoved(prev.hash, "replaced")
		p.lookup.track(meta)
...
func (p *BlobPool) trackRemoved(hash common.Hash, reason string) {
